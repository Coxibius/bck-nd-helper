...
def parse_project_for_php_uml(
    ...
    *,
    ...
    candidates = (
        ((path, path.relative_to(root)) for path in file_index.php_files)
        if file_index is not None
        else walk_source_files(str(root), (".php",), max_depth=max_depth)
    )
